...
class Citas_confirmadas extends BaseController
{
    ...
    {
        $session = session();
        if (!Permisos::is_rol_permitido(TAREA_CITAS_CONFIRMADAS, isset($session->rol_actual['clave']) ? $session->rol_actual['clave'] : -1)) {
            $this->permitido = false;
        } //end if rol no permitido
        else {
            $session->tarea_actual = TAREA_CITAS_CONFIRMADAS;
    ...
    {
        if ($this->permitido) {
            return $this->crear_vista("panel/citas_confirmadas", $this->cargar_datos());
    ...
    private function cargar_datos()
    {
        ...
        //======================================================================
        $datos['nombre_pagina'] = 'Citas confirmadas';

        //Cargar modelos
        $tabla_Citas = new \App\Models\Tabla_Citas;
        $datos['citas'] = $tabla_Citas->datatable_citas_confirmadas();

        //Breadcrumb
        $navegacion = array(
            array(
                'tarea' => 'Citas confirmadas',
                'href' => '#'
            )
        );
        $datos['breadcrumb'] = breadcrumb_panel($navegacion, 'Citas confirmadas');
