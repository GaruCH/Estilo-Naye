...
class Citas_canceladas extends BaseController
{
    ...
    {
        $session = session();
        if (!Permisos::is_rol_permitido(TAREA_CITAS_CANCELADAS, isset($session->rol_actual['clave']) ? $session->rol_actual['clave'] : -1)) {
            $this->permitido = false;
        } //end if rol no permitido
        else {
            $session->tarea_actual = TAREA_CITAS_CANCELADAS;
    ...
    {
        if ($this->permitido) {
            return $this->crear_vista("panel/citas_canceladas", $this->cargar_datos());
    ...
    private function cargar_datos()
    {
        ...
        //======================================================================
        $datos['nombre_pagina'] = 'Citas canceladas';

        //Cargar modelos
        $tabla_Citas = new \App\Models\Tabla_Citas;
        $datos['citas'] = $tabla_Citas->datatable_citas_canceladas();

        //Breadcrumb
        $navegacion = array(
            array(
                'tarea' => 'Citas canceladas',
                'href' => '#'
            )
        );
        $datos['breadcrumb'] = breadcrumb_panel($navegacion, 'Citas canceladas');
